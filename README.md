...
- 提供API: `export-equipments`（設備一覧）/ `export-individual-installations`（個体設置明細）/ `export-individual-attributes`（個体属性値一覧・縦持ち）/ `export-equipment-individual-attributes`（設備個体属性値・横持ち）
- 4件ともSupabaseにデプロイ済み（`supabase/functions/`配下にソースを保存）。APIキー未設定の間はすべて401を返す。運用開始前に `supabase secrets set --project-ref nbahkykcxlulzkafrmkf EXTERNAL_API_KEY=<値>` でキーを設定する必要がある。
