...
デモWebアプリ（ログイン+CRUD+台帳確認画面+設備交換履歴画面）はGitHub Pagesで公開中。

## 設備交換履歴画面

「個体設置履歴と設備台帳の関係が分かりにくい」という指摘を受けて追加した画面。設備を選択すると、構成要素（主装置・付属部品）ごとに、初回設置から現在までの個体の交換を「旧→新」のカード形式で時系列表示する。`view_equipment_installation_histories`（0005）から取得した履歴を、構成要素ごとに設置日でソートし、隣り合う2件を交換ペアとして組み立てている。
...
- 台帳確認・設備交換履歴画面の絞り込み条件の拡充（場所での検索を追加）
