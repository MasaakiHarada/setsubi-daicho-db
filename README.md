...
docs/
  ...
  外部インターフェース仕様_API.md -- 外部連携APIの仕様書
webapp/
  index.html               -- デモWebアプリ（docs/index.htmlと同一内容）
supabase/
  functions/                -- 外部連携API（Supabase Edge Functions、デプロイ済み）
```
...
台帳確認画面のExcel出力機能（設備一覧・個体設置明細・個体属性値一覧・設備個体属性値）を、外部システム連携用にSupabase Edge Functions（Deno）としてAPI化した。Excelファイルではなく、JSON（項目名・値）で返却する。詳細仕様は [`docs/外部インターフェース仕様_API.md`](docs/外部インターフェース仕様_API.md) を参照。

- ベースURL: `https://nbahkykcxlulzkafrmkf.supabase.co/functions/v1/`
- 認証: リクエストヘッダー `X-API-Key` による専用APIキー認証（Supabaseログインユーザーとは無関係。Edge Function内部はservice_roleでDBアクセス）
- 提供API: `export-equipments`（設備一覧）/ `export-individual-installations`（個体設置明細）/ `export-individual-attributes`（個体属性値一覧・縦持ち）/ `export-equipment-individual-attributes`（設備個体属性値・横持ち）/ `export-removed-individuals`（撤去済み個体一覧、MCPサーバ化プロジェクトで追加）
- 5件ともSupabaseにデプロイ済み（`supabase/functions/`配下にソースを保存）。APIキー未設定の間はすべて401を返す。運用開始前に `supabase secrets set --project-ref nbahkykcxlulzkafrmkf EXTERNAL_API_KEY=<値>` でキーを設定する必要がある。
- 動作確認用スクリプト: `scripts/test_api.ps1`（PowerShell）。`.\test_api.ps1 -ApiKey "<APIキー>"` で4APIの正常系・異常系（401/400/404）をまとめて実行できる。
...
- 外部インターフェースAPI: APIキーのローテーション・複数キー発行・レート制限
