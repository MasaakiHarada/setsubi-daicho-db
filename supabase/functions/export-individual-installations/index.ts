// API2: 個体設置明細出力API
// GET /functions/v1/export-individual-installations?search=&system_category_kbn=&equipment_type_id=
// 現在設置されている個体のみを対象とする（設備側の絞り込みはAPI1と同じ条件）。
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const equipmentNameMap = new Map(equipments.map((e: any) => [e.id, e.equipment_name]));
    ...
    const items = (curInd || []).map((r: any) => ({
      equipment_id: r.equipment_id,
      equipment_name: equipmentNameMap.get(r.equipment_id) ?? null,
